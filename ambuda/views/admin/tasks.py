...
def edit_tags(model_name, selected_ids: list | None = None):
    """Batch action to add/remove tags for multiple texts."""
    from ambuda.models.texts import text_tag_association
    ...
    all_tags = session.query(db.TextTag).order_by(db.TextTag.name).all()

    if request.form.get("save_tags") and form.validate_on_submit():
        add_tag_ids = set(request.form.getlist("add_tags", type=int))
        remove_tag_ids = set(request.form.getlist("remove_tags", type=int))

        try:
            text_ids = [int(tid) for tid in selected_ids]

            if remove_tag_ids:
                session.execute(
                    text_tag_association.delete().where(
                        text_tag_association.c.text_id.in_(text_ids),
                        text_tag_association.c.tag_id.in_(remove_tag_ids),
                    )
                )
            ...
            if add_tag_ids:
                existing = set(
                    session.execute(
                        select(
                            text_tag_association.c.text_id,
                            text_tag_association.c.tag_id,
                        ).where(
                            text_tag_association.c.text_id.in_(text_ids),
                            text_tag_association.c.tag_id.in_(add_tag_ids),
                        )
                    ).all()
                )
                for text_id in text_ids:
                    for tag_id in add_tag_ids:
                        if (text_id, tag_id) not in existing:
                            session.execute(
                                text_tag_association.insert().values(
                                    text_id=text_id,
                                    tag_id=tag_id,
                                    is_featured=False,
                                )
            ...
            session.commit()
            flash(
                f"Tags updated for {len(text_ids)} text(s): "
                f"{len(add_tag_ids)} added, {len(remove_tag_ids)} removed",
                "success",
            )
            return redirect(url_for("admin.list_model", model_name=model_name))
